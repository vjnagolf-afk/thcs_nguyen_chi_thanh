...
# ============================================================
# 2. IMPORT CÁC THƯ VIỆN / MODULE HỆ THỐNG
# ============================================================
...
# ============================================================
# 3. IMPORT PHÂN HỆ HỖ TRỢ GIÁO VIÊN
# ============================================================
...
# ============================================================
# 4. IMPORT PHÂN HỆ HỖ TRỢ GIẢNG DẠY
# ============================================================
...
from modules.ho_tro_giang_day.xd_hoc_lieu import (
    render_xd_tuong_tac
)

# MÔ PHỎNG CŨ
from modules.ho_tro_giang_day.xd_mo_phong import (
    render_xd_mo_phong
)

from modules.ho_tro_giang_day.xd_phan_tich import (
    render_xd_phan_tich
)

from modules.ho_tro_giang_day.xd_ngan_hang_de import (
    render_xd_ngan_hang_de
)

from modules.ho_tro_giang_day.xd_sinh_video import (
    render_xd_sinh_video
)

from modules.ho_tro_giang_day.xd_camera import (
    render_camera_module
)

from modules.ho_tro_giang_day.xd_ca_nhan_hoa import (
    render_xd_ca_nhan_hoa
)

from modules.ho_tro_giang_day.xd_phan_tich_bh import (
    render_phan_tich_bh
)

from modules.ho_tro_giang_day.xd_kiem_tra_nhanh import (
    render_kiem_tra_nhanh
)


# ============================================================
# 5. IMPORT PHÂN HỆ QUẢN LÝ TỔ CHUYÊN MÔN
# ============================================================

from modules.quan_ly_to.danh_sach import (
    render_danh_sach
)

from modules.quan_ly_to.phan_cong import (
    render_phan_cong
)

from modules.quan_ly_to.bien_ban import (
    render_bien_ban
)

from modules.quan_ly_to.xd_ke_hoach import (
    render_ke_hoach
)

from modules.quan_ly_to.xd_thi_dua import (
    render_thi_dua
)

from modules.quan_ly_to.xd_kiem_tra_khbd import (
    render_kiem_tra_khbd
)

from modules.quan_ly_to.xd_sach_kn_so import (
    render_sach_kn_so
)

from modules.quan_ly_to.xd_tom_tat_gmail import (
    render_tom_tat_gmail
)

from modules.quan_ly_to.xd_viet_sang_kien import (
    render_viet_sang_kien
)

from modules.quan_ly_to.xd_cham_sang_kien import (
    render_cham_sang_kien
)
...
def get_ai_engine_instance():
    ...
    if "ai_engine_instance" not in st.session_state:
        ...
        if st.session_state.is_admin_mode:
            ...
            keys["gemini"] = st.secrets.get(
                "GEMINI_API_KEY"
            )

            keys["openai"] = st.secrets.get(
                "OPENAI_API_KEY"
            )
        ...
        if keys:
            ...
            st.session_state.ai_engine_instance = AIEngine(
                keys=keys
            )
...
# ============================================================
# 9. KHỞI TẠO AI ENGINE 2 - OPENROUTER
# ============================================================
...
with st.sidebar:
    ...
    st.markdown(
        """
        <h2 style='text-align: center; color: #E63946;'>
            HỆ SINH THÁI SỐ
        </h2>
        """,
    ...
    if (
        ...
        or st.session_state.is_admin_mode
    ):
...
if (
    ...
    and not st.session_state.is_admin_mode
):

    st.info(
        "🔑 Vui lòng nhập API Key / Mật khẩu hệ thống để bắt đầu."
    )

    with st.form("login_form"):

        key_input = st.text_input(
            "Nhập API Key / Mật khẩu:",
            type="password"
        )
...
# ============================================================
    ...
    st.error(
        "🚨 Có lỗi xảy ra trong quá trình render!"
    )
