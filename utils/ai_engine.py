...
"""
            ...
            if provider == "gemini":
                ...
                if openai:
                    self.openrouter_clients[key] = openai.Client(
                        api_key=key,
                        base_url="https://openrouter.ai/api/v1", # Đã fix URL OpenRouter chuẩn
                        timeout=60
                    )
    ...
    def _call_provider(self, provider, api_key, prompt, **kwargs):
        if provider == "gemini":
            ...
            return (response.choices[0].message.content, model_name) # Đã thêm [0]
            ...
            return (response.content[0].text, model_name) # Đã thêm [0]
