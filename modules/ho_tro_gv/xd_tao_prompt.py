...
# ============================================================
# HÀM GỌI AI THÔNG MINH (CROSS-ROUTING FALLBACK) - DÙNG CHUNG CHO CẢ 2 TAB
# ============================================================
def call_prompt_engineer(ai_engine, prompt, model_name="Gemini 1.5 Flash"):
    ...
    def run_gemini():
        try:
            ...
            if res and not res.startswith("❌") and not res.startswith("⚠️") and "429" not in res and "RESOURCE_EXHAUSTED" not in res:
                return res
            raise RuntimeError(f"Hạn mức Gemini cạn kiệt.")
        except ImportError:
            if ai_engine and hasattr(ai_engine, "generate_text"):
                ...
                if res and not res.startswith("❌") and not res.startswith("⚠️") and "429" not in res and "RESOURCE_EXHAUSTED" not in res:
                    ...
                    return res
                raise RuntimeError("Hạn mức Gemini Engine 1 cạn kiệt.")
        except Exception as e:
            raise RuntimeError(f"Lỗi từ máy chủ Google: {str(e)}")
    ...
    if is_openai_preferred:
        try: return run_openai()
            ...
            error_messages.append(f"OpenAI: {e1}")
            try: return run_gemini()
            except Exception as e2: error_messages.append(f"Gemini: {e2}")
    else:
        try: return run_gemini()
        except Exception as e1: 
            error_messages.append(f"Gemini: {e1}")
            try: return run_openai()
...
# ============================================================
# GIAO DIỆN CHÍNH DÙNG TABS
# ============================================================
def render_xd_tao_prompt(ai_engine=None):
    st.title("🧠 Không Gian Sáng Tạo & Tối Ưu Prompt")
    
    # TÁCH LÀM 2 TAB NHƯ YÊU CẦU CỦA THẦY
    tab1, tab2 = st.tabs(["🚀 MÁY PHÁT SINH PROMPT NHANH (MỚI)", "🛠️ ĐÚC KẾT META PROMPT (CŨ)"])
    
    # ---------------------------------------------------------
    # TAB 1: TÍNH NĂNG MỚI ĐÃ NÂNG CẤP
    # ---------------------------------------------------------
    with tab1:
        ...
        with col1:
            ...
            nhom_ai = st.selectbox("Bạn đang muốn sử dụng nhóm AI nào?", list(AI_CATEGORIES.keys()), key="tab1_nhom")
            model_name_tab1 = st.selectbox("Động cơ sinh Prompt:", ["Gemini 1.5 Flash (Tốc độ)", "Gemini 1.5 Pro (Sâu sắc)", "GPT-4o Mini", "GPT-4o (Cao cấp)"], key="tab1_model")
        ...
        if btn_prompt:
            if not chi_tiet.strip():
                ...
                with st.spinner(f"Đang đúc khuôn Meta-Template cho {nhom_ai}..."):
                    ...
                    try:
                        res = call_prompt_engineer(ai_engine, prompt_engineer_task, model_name_tab1)
                        st.markdown("---")
    ...
    # ---------------------------------------------------------
    # TAB 2: TÍNH NĂNG CŨ ĐÃ ĐƯỢC CHỮA LỖI 429
    # ---------------------------------------------------------
    with tab2:
        st.caption("Giao diện cũ của thầy đã được bọc thuật toán chống lỗi 429.")
        
        c1, c2 = st.columns(2)
        with c1:
            loai_ky_thuat = st.selectbox("Kỹ thuật Prompt:", ["Zero-shot (Yêu cầu trực tiếp, không cần mẫu)", "Few-shot (Đưa mẫu)", "Chain of Thought (Suy luận)"])
        with c2:
            vai_tro = st.selectbox("Vai trò chuyên gia:", ["Chuyên gia giáo dục nghề nghiệp", "Chuyên gia sư phạm", "Nhà tâm lý học", "Chuyên gia CNTT"])
            
        st.markdown("### 2. Cốt lõi Yêu cầu")
        cot_loi = st.text_area("Thầy/Cô muốn AI làm công việc gì cụ thể?", height=120, placeholder="Định hướng nghề nghiệp sớm giúp học sinh lựa chọn học tiếp lên THPT hoặc học nghề...")
        
        btn_meta = st.button("🪄 ĐÚC KẾT SIÊU CÂU LỆNH (META PROMPT)", type="primary", use_container_width=True, key="btn_meta_old")
        
        if btn_meta:
            if not cot_loi.strip():
                st.warning("⚠️ Vui lòng nhập cốt lõi yêu cầu.")
            else:
                with st.spinner("Đang phân tích và đúc kết Meta Prompt..."):
                    prompt_old_logic = f"Kỹ thuật: {loai_ky_thuat}. Đóng vai: {vai_tro}. Yêu cầu cốt lõi: {cot_loi}. Hãy viết một siêu câu lệnh (Meta Prompt) tối ưu nhất."
                    try:
                        # CHÌA KHÓA Ở ĐÂY: Sử dụng call_prompt_engineer thay vì ai_engine.generate_text trực tiếp để tránh lỗi 429
                        res_old = call_prompt_engineer(ai_engine, prompt_old_logic, model_name="Gemini 1.5 Flash")
                        st.markdown("---")
                        st.success("✅ Đã đúc kết thành công! Tránh được lỗi 429 nhờ thuật toán chuyển hướng thông minh.")
                        st.markdown(res_old)
                    except Exception as e:
