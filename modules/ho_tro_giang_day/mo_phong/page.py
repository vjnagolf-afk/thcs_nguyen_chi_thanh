...
def _extract_html_code(text):
    """Hàm trích xuất mã HTML sạch từ câu trả lời của AI"""
    if not text: return ""
...
def render_mo_phong(ai_engine):
    ...
    with tab_ai:
        ...
        with st.form("form_tao_mo_phong"):
            ...
            with col1:
                ten_mp = st.text_input("Tên hiện tượng / Bài học (VD: Con lắc đơn, Phản ứng Oxi hóa):")
            ...
            with col2:
                ...
                yeu_cau_them = st.text_input("Yêu cầu bổ sung (Tùy chọn, VD: Có thanh trượt chỉnh khối lượng):")
            
            # 2 Nút bấm chạy 2 luồng khác nhau
        ...
        if st.session_state.code_mo_phong:
            ...
            # Dùng Streamlit Components để render HTML ngay trên app
    ...
    with tab_nhung:
        ...
        st.info("💡 Lời khuyên: Để các mô phỏng chạy mượt mà và không bị chặn tương tác bởi trình duyệt (lỗi iframe chặn chéo), thầy cô nên ấn mở trong thẻ mới (Tab mới).")
        ...
        with col_phet:
            ...
            st.write("Kho mô phỏng tương tác khoa học Tự nhiên và Toán học hàng đầu thế giới, hoàn toàn miễn phí và có tiếng Việt.")
            st.link_button("🚀 Mở PhET Tiếng Việt", "https://phet.colorado.edu/vi/", use_container_width=True)

        with col_moza:
            st.markdown("### 🧬 MozaWeb 3D")
            st.markdown("**Thư viện Giáo dục Mozaik**")
            st.write("Hàng ngàn cảnh 3D tương tác sắc nét, video giáo dục và ứng dụng cho Sinh học, Lịch sử, Địa lý...")
            st.link_button("🌐 Mở MozaWeb 3D", "https://mozaweb.vn/vi/lexikon.php?cmd=getlist&let=3D&sid=BIO", use_container_width=True)
