                ...
                if file_khbd:
                    with st.spinner("AI đang đọc file, trích xuất dữ liệu và lập bảng phân tích..."):
                        try:
                            ...
                            - 🔤 Lỗi chính tả & Câu từ: 
                              [LƯU Ý]: Nếu thấy các từ có khoảng trắng bất thường (như 'Ho ạt động'), hãy tự động ngầm ghép chúng lại và bỏ qua lỗi này. CHỈ báo lỗi khi sai âm vần thực sự (VD: 'xắp xếp', 'năng lự').
        ...
        with tab_chat:
            ...
            with q1:
                if st.button("🎯 Kiểm tra Mục tiêu (Bloom)", use_container_width=True): 
                    quick_prompt = "Hãy rà soát xem phần Mục tiêu đã dùng đúng động từ Bloom chưa? Nếu sai, hãy trích dẫn câu sai và gợi ý cách sửa."
                if st.button("🔄 Đề xuất lại HĐ Vận dụng", use_container_width=True): 
                    quick_prompt = "Dựa vào nội dung bài này, hãy đề xuất một Hoạt động Vận dụng gắn với thực tiễn đời sống để bổ sung vào giáo án."
            with q2:
                    ...
                    with chat_container:
                        ...
                        with st.chat_message("assistant"):
                            with st.spinner("AI đang đọc lại giáo án và trả lời..."):
                                try:
                                    chat_prompt = f"LƯU Ý: Bỏ qua lỗi khoảng trắng ngẫu nhiên. Dựa vào nội dung KHBD sau:\n\n{st.session_state.noidung_khbd}\n\nThực hiện yêu cầu sau: {prompt}"
