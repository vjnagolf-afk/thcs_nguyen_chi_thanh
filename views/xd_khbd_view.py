...
def _show_source_quality(text, title="Nguồn kiến thức chính"):
    """Hiển thị thống kê dữ liệu nguồn cho giáo viên."""
    ...
    if quality["status"] == "empty":
        st.error("❌ Không đọc được nội dung văn bản từ tài liệu nguồn.")
    elif quality["status"] == "very_low":
        st.warning("⚠️ Nội dung trích xuất được rất ít. PDF có thể là bản scan, ảnh hoặc file bị lỗi lớp văn bản.")
    elif quality["status"] == "low":
        st.warning("⚠️ Nội dung nguồn còn ngắn. Nên kiểm tra lại phạm vi trang hoặc khả năng trích xuất.")
    else:
        st.success("✅ Dữ liệu nguồn đã được đọc và sẵn sàng đưa vào AI.")
...
def render_xd_khbd(ai_engine=None):
    ...
    with col2:
        mon_hoc = st.selectbox(
            "Môn học",
            ["Toán", "Ngữ văn", "Tiếng Anh", "Khoa học tự nhiên", "Vật lí", "Hóa học", "Sinh học", "Lịch sử và Địa lí", "Tin học", "Công nghệ", "Khác"],
            key="khbd_mon_hoc",
        )
    ...
    mode = st.radio(
        "Chọn chế độ",
        ["chinh_sua", "tu_dong"],
        format_func=lambda x: "📄 Chỉnh sửa giáo án gốc" if x == "chinh_sua" else "⚡ Tự động soạn từ SGK",
        key="khbd_mode",
        horizontal=True,
    )
    ...
    range_trang = ""
    if mode == "tu_dong":
        st.info("💡 Nếu chỉ soạn một bài trong SGK, nên giới hạn phạm vi trang để AI tập trung phân tích nội dung.")
    ...
    with col_td1:
        ten_bai = st.text_input("Tên bài dạy", key="khbd_ten_bai")

    with col_td2:
        so_tiet = st.text_input("Thời lượng tiết học", value="1 tiết", key="khbd_so_tiet")
    ...
    with c_th1:
        tich_hop_nls = st.checkbox("Năng lực số (TT18)", key="khbd_tich_hop_nls")

    with c_th2:
        tich_hop_ai = st.checkbox("Năng lực AI", key="khbd_tich_hop_ai")

    with c_th3:
        tich_hop_hoa_nhap = st.checkbox("Dạy học hòa nhập", key="khbd_tich_hop_hoa_nhap")
    ...
    with c_hd2:
        st.button("➕ Thêm", on_click=add_activity, use_container_width=True)
    ...
    if tich_hop_nls:
        with st.container(border=True):
            ...
            loai_khung = st.radio(
                "Chuẩn",
                ["Giáo viên (Thông tư 18)", "Học sinh (DigComp)"],
                horizontal=True,
            ...
            with col_lv:
                linh_vuc = st.selectbox("Lĩnh vực", domains, key="khbd_nls_linh_vuc")

            components = get_nls_components(loai_khung, linh_vuc)
            with col_tp:
                thanh_phan = st.selectbox("Thành phần", components, key="khbd_nls_thanh_phan")

            levels = get_nls_levels(loai_khung, linh_vuc, thanh_phan)
            with col_md:
                muc_do = st.selectbox("Mức độ", levels, key="khbd_nls_muc_do")

            tu_dong_noi_dung = get_nls_content(loai_khung, linh_vuc, thanh_phan, muc_do)
    ...
    if st.button("⚡ KÍCH HOẠT XỬ LÝ AI", type="primary", use_container_width=True):
        if ai_engine is None:
            ...
            with st.spinner("🧠 Hệ thống đang phân tích tài liệu và biên soạn KHBD..."):
                try:
                    noi_dung_chinh = read_multiple_files(file_sgk, range_trang, is_pdf_target=True) if mode == "tu_dong" else ""
                    ...
                    if mode == "tu_dong":
                        _show_source_quality(noi_dung_chinh, "SGK / Tài liệu kiến thức")
                        if not noi_dung_chinh.strip():
                            st.error("❌ Không đọc được nội dung SGK.")
                            st.stop()

                    thong_tin = (
                        f"- Khối: {khoi_lop}\n"
                        f"- Môn: {mon_hoc}\n"
                        f"- Tên bài: {ten_bai or 'Theo nguồn'}\n"
                        f"- Số tiết: {so_tiet}\n"
                        f"- Ngôn ngữ: {'English' if tieng_anh else 'Tiếng Việt'}"
                    )
                    ...
                    st.error(f"❌ Lỗi kiểm định dữ liệu nguồn: {ve}")
                except Exception as e:
                    st.error(f"❌ Lỗi hệ thống: {e}")

    # 11. HIỂN THỊ KẾT QUẢ & XUẤT WORD
    ...
    if result:
        ...
        if WordExportEngine is None:
            st.error("❌ Không kết nối được module `export.word_export_engine`.")
        else:
            try:
                ...
                try:
                    word_bytes = WordExportEngine.convert_markdown_to_docx_bytes(
                        result,
