...
def render_xd_khbd(
    ...
    if mode == "tu_dong":
        ...
        st.info(
            "💡 Nếu SGK là PDF scan, hệ thống sẽ "
            "thử OCR khi không đọc được lớp văn bản."
        )
    ...
    st.subheader(
        "📌 Hoạt động giáo viên mong muốn thêm"
    )

    c_hd1, c_hd2 = st.columns(
        [
            5,
            1
        ]
    ...
    if st.button(
        ...
        with st.spinner(
            "🧠 Đang đọc nguồn → "
            "kiểm định → lập phạm vi kiến thức → "
            "phân bổ số tiết → biên soạn..."
        ):
            ...
            try:
                ...
                if mode == "tu_dong":
                    ...
                    quality = inspect_source_quality(
                        noi_dung_chinh,
                        "Tài liệu SGK"
                    )

                    st.session_state.khbd_source_quality = (
                        quality
                    )
                ...
                expected_lessons = extract_lesson_count(
                    thong_tin
                )
