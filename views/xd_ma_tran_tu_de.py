            ...
            if file_name.endswith(".txt"):
                for encoding in ("utf-8", "utf-8-sig", "cp1258"):
                    try:
                        ...
                        continue
                raise ValueError("Không thể giải mã tệp TXT.")

            raise ValueError(f"Định dạng không được hỗ trợ: {file_name}")

        except Exception as e:
            raise RuntimeError(f"Lỗi đọc tệp {file_name}: {e}")
...
class MatrixCalculator:
    ...
    def normalize_matrix(rows):
        ...
        for item in rows:
            ...
            result.append({
                "chu_de": str(item.get("chu_de", "")).strip(),
    ...
    @staticmethod
    def normalize_specification(rows):
        result = []
        if not isinstance(rows, list): return result
        ...
        for index, item in enumerate(rows, start=1):
            ...
            result.append({
                ...
                "chu_de": str(item.get("chu_de", "")).strip(),
                "bai_hoc": str(item.get("bai_hoc", item.get("noi_dung", ""))).strip(),
                "yccd": str(item.get("yccd", "")).strip(),
    ...
    @staticmethod
    def prepare_context(parsed_data, mon_hoc, lop):
        if not isinstance(parsed_data, dict):
            raise ValueError("Dữ liệu AI không đúng cấu trúc.")

        ma_tran = MatrixCalculator.normalize_matrix(parsed_data.get("ma_tran", []))
        dac_ta = MatrixCalculator.normalize_specification(parsed_data.get("dac_ta", []))
...
def render_xd_ma_tran_tu_de(ai_engine):
    ...
    if st.button("PHÂN TÍCH ĐỀ & LẬP MA TRẬN", type="primary", use_container_width=True):
        if not file_de:
            st.warning("Vui lòng tải lên đề kiểm tra.")
            return

        try:
            with st.spinner("AI đang phân tích đề kiểm tra..."):
                raw_text = ExamTextExtractor.extract(file_de)
                exam_text = ExamTextExtractor.normalize(raw_text)

                if not exam_text:
                    raise ValueError("Không đọc được nội dung đề.")
...
"""
                ...
                prompt = f"""
BẠN LÀ CHUYÊN GIA KHẢO THÍ THEO CHƯƠNG TRÌNH GDPT 2018.

Hãy phân tích đề kiểm tra môn {mon_hoc}, {lop}.

MỤC TIÊU:
1. Xác định toàn bộ nội dung kiến thức thực sự xuất hiện trong đề.
2. Phân tích đầy đủ từng câu hỏi.
3. Phân loại đúng mức độ: nb (Nhận biết), th (Thông hiểu), vd (Vận dụng), vdc (Vận dụng cao).
4. Không được bỏ sót câu hỏi.
5. Không được thêm nội dung không xuất hiện trong đề.
...
CHỈ TRẢ VỀ JSON.
"""
                # Tương thích với AI Engine Wrapper
                ...
                context = MatrixCalculator.prepare_context(parsed_data, mon_hoc, lop)

                st.session_state["processed_matrix_data"] = context
                st.session_state["mt_mon_hoc_file"] = mon_hoc
                st.session_state["mt_lop_file"] = lop

                if export_word is None:
                    raise ImportError("export_word chưa được import.")

                word_bytes = export_word(
                    data=context,
                    template_name="ma_tran_dac_ta_mau.docx"
                )

                st.session_state["download_word_bytes"] = word_bytes
                st.success("🎉 Đã phân tích và xuất Word thành công.")

        except json.JSONDecodeError:
            st.error("❌ AI trả về JSON không hợp lệ.")
        except Exception as e:
    ...
    if data:
        ...
        if data.get("MA_TRAN"):
            ...
            st.dataframe(pd.DataFrame(data["MA_TRAN"]), use_container_width=True)

        if data.get("DAC_TA"):
            st.markdown("**2. ĐẶC TẢ**")
            st.dataframe(pd.DataFrame(data["DAC_TA"]), use_container_width=True)
