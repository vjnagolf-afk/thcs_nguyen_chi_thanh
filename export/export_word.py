...
"""
============================================================
MODULE: export/export_word.py (ĐÃ BỌC BẪY LỖI FALLBACK AN TOÀN TUYỆT ĐỐI)
============================================================
"""
...
# ============================================================
# 1. BỘ PHÂN TÍCH MARKDOWN NỘI BỘ
# ============================================================
...
# ============================================================
# 2. KẾT XUẤT TÀI LIỆU WORD (CÓ BẪY LỖI DỰ PHÒNG)
# ============================================================
class WordExportEngine:
    ...
    def _render_khbd_header(cls, doc, metadata: dict):
        try:
            ...
            rt = p_title.add_run(f"TÊN BÀI DẠY: {str(metadata.get('title', '.........................')).upper()}")
            rt.bold = True
            rt.font.size = Pt(16)
            cls._set_font(rt)
    ...
    def convert_markdown_to_docx_bytes(cls, markdown_text: str, metadata: dict = None) -> bytes:
        try:
            ...
            for node in ast_nodes:
                try:
                    ...
                    if nt == "paragraph":
                        ...
                        st_style = 'List Number' if node.get("style") == "number" else 'List Bullet'
                        p = doc.add_paragraph(style=st_style)
            ...
            # === FALLBACK AN TOÀN TUYỆT ĐỐI (ĐẢM BẢO KHÔNG BAO GIỜ LỖI) ===
            fallback_doc = docx.Document()
            fallback_doc.add_paragraph("KẾ HOẠCH BÀI DẠY (BẢN XUẤT NHANH)")
            for line in (markdown_text or "").splitlines():
                if line.strip():
                    fallback_doc.add_paragraph(line.strip().replace('**', ''))
            bio = io.BytesIO()
...
# ============================================================
# 3. PUBLIC API CŨ (TƯƠNG THÍCH MỌI KIỂU GỌI)
# ============================================================
def export_word(markdown_text_or_cache) -> bytes:
    try:
        ...
        doc = docx.Document()
        doc.add_paragraph(f"Nội dung giáo án:\n{str(markdown_text_or_cache)}")
        bio = io.BytesIO()
        doc.save(bio)
        return bio.getvalue()
