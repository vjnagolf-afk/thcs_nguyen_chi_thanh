...
class WordExportEngine:
    ...
    def _set_font(run, font_name="Times New Roman"):
        """Khóa cứng Font XML thông minh - Sửa lỗi tạo trùng thẻ rFonts"""
    ...
    def _set_shading(p_or_cell, color_hex):
        ...
        if shd is None: shd = OxmlElement("w:shd"); element.append(shd)
        shd.set(qn("w:val"), "clear"); shd.set(qn("w:fill"), color_hex)
    ...
    def _render_inline(cls, p, tokens: List[Dict[str, Any]]):
        ...
        for t in tokens:
            ...
            if tt in ["text", "bold", "italic", "underline", "strike", "highlight", "subscript", "superscript", "inline_code"]:
                ...
                elif tt == "inline_code": run.font.size = Pt(11); run.font.color.rgb = RGBColor(199, 37, 78)
            elif tt == "inline_math":
                run = p.add_run(ScienceNormalizer.normalize(c))
                run.font.italic = True; cls._set_font(run, "Times New Roman")
    ...
    def convert_markdown_to_docx_bytes(cls, markdown_text: str) -> bytes:
        ...
        # Thiết lập lề trang A4 chuẩn hành chính bộ giáo dục
        for s in doc.sections:
            ...
            s.left_margin = Inches(1.18) # Trái 3cm rộng rãi để đóng ghim giáo án

        # Khởi tạo định dạng văn bản mặc định toàn văn
        ...
        for node in ast_nodes:
            ...
            if nt == "paragraph":
                ...
                p = doc.add_paragraph()
                p.paragraph_format.space_before, p.paragraph_format.space_after, p.paragraph_format.keep_with_next = Pt(10), Pt(4), True
                if lv == 1: p.alignment = WD_ALIGN_PARAGRAPH.CENTER
                cls._render_inline(p, node.get("tokens", []))
                for r in p.runs: r.bold = True; r.font.size = Pt(17 - lv); cls._set_font(r)
            elif nt == "list_item":
                st = 'List Number' if node.get("style") == "number" else 'List Bullet'
                p = doc.add_paragraph(style=st)
                p.paragraph_format.left_indent, p.paragraph_format.first_line_indent = Inches(0.25 * node.get("level", 1) + 0.25), Inches(-0.25)
                cls._render_inline(p, node.get("tokens", []))
            elif nt == "checkbox":
                p = doc.add_paragraph()
                ...
                bottom.set(qn("w:val"), "single"); bottom.set(qn("w:sz"), "8"); bottom.set(qn("w:color"), "CCCCCC")
                ...
                table.style = 'Table Grid'; table.alignment = 1
                r_idx = 0
                if headers:
                    h_row = table.rows[0]; h_row._element.get_or_add_trPr().append(OxmlElement('w:tblHeader'))
                    for c_idx, cell_n in enumerate(headers):
                        cell = h_row.cells[c_idx]; cls._set_shading(cell, "EAEAEA")
                        p = cell.paragraphs[0]; cls._render_inline(p, cell_n.get("content", []))
                        for r in p.runs: r.bold = True
                    r_idx = 1
                for loop_idx, r_data in enumerate(rows):
                    row = table.rows[r_idx]; row._element.get_or_add_trPr().append(OxmlElement('w:cantSplit'))
                    bg = "F9F9F9" if loop_idx % 2 == 1 else "FFFFFF"
                    for c_idx, cell_n in enumerate(r_data):
                        cell = row.cells[c_idx]; cls._set_shading(cell, bg)
