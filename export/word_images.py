...
def insert_image_to_paragraph(paragraph, image_source, width=None, height=None, max_width=Inches(6.0)):
    ...
    Xử lý thông minh kích thước và chèn chú thích (Caption) từ Metadata.
    """
    try:
        ...
        if not width and not height:
            try:
                ...
                # Nếu ảnh nhỏ hơn max_width, python-docx sẽ giữ nguyên kích thước native gốc
            except ImportError:
                # Fallback nếu hệ thống không cài Pillow (PIL) -> Ép max_width để giữ an toàn lề
...
def insert_image_to_docx(doc: Document, image_path_or_url: str):
    ...
    Được gọi tự động bởi `export_word.py` khi gặp thẻ `![...](...)` trong Markdown.
    """
...
def insert_image_to_cell(cell, image_source, width=None, height=None, max_width=Inches(2.5)):
    """
    API Bổ trợ: Chèn hình ảnh an toàn vào bên trong một ô (Cell) của Bảng (Table).
    Tự động giới hạn max_width hẹp lại để không làm vỡ cấu trúc cột.
    """
    if not image_source:
        return
    ...
    p.alignment = WD_ALIGN_PARAGRAPH.CENTER
    return insert_image_to_paragraph(p, image_source, width=width, height=height, max_width=max_width)
