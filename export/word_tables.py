...
class BaseStyleSetup:
    ...
    def setup_base_styles(doc: docx.Document):
        # 1. Cấu hình lề trang A4 chuẩn hành chính
        ...
        # 2. Cấu hình mẫu phông chữ hệ thống toàn cục
        styles_config = {
...
class XmlHelpers:
    ...
    def set_font_safely(run, font_name: str = "Times New Roman"):
        """Sửa lỗi trùng lặp thẻ rFonts khi gọi đè nhiều lần trên cùng một Run"""
    ...
    def apply_paragraph_shading(paragraph, color_hex: str = "F5F5F5"):
        """Đổ màu nền nguyên khối cho cả đoạn văn (dùng cho Khối mã/Callout)"""
    ...
    def apply_bottom_border(paragraph, color_hex: str = "CCCCCC", size: int = 8):
        """Tạo đường kẻ ngang ngăn cách (HR) bằng Paragraph Border tinh tế"""
        pPr = paragraph._element.get_or_add_pPr()
...
class BlockRenderer:
    @classmethod
    def render_heading(cls, doc, node: dict, text_renderer: any, math_renderer: any):
    ...
    @classmethod
    def render_list_item(cls, doc, node: dict, text_renderer: any, math_renderer: any):
        style_name = 'List Number' if node.get("style") == "number" else 'List Bullet'
        p = doc.add_paragraph(style=style_name)
        
        # Thụt lề treo (Hanging Indent) chuẩn chỉnh cho dòng văn bản dài
    ...
    @classmethod
    def render_checkbox(cls, doc, node: dict, text_renderer: any, math_renderer: any):
        p = doc.add_paragraph()
        ...
        XmlHelpers.set_font_safely(run_box, "MS Gothic")  # Font chuẩn hiển thị ô vuông
        run_box.bold = True
...
class ContainerRenderer:
    ...
    @classmethod
    def render_callout(cls, doc, node: dict, text_renderer: any, math_renderer: any):
        ...
        # Đổ màu nền ô và cấu hình viền trái dày
        shd = OxmlElement('w:shd')
        shd.set(qn('w:val'), 'clear'); shd.set(qn('w:fill'), bg_color)
        tcPr.append(shd)
        
        borders = OxmlElement('w:tcBorders')
        left_b = OxmlElement('w:left'); left_b.set(qn('w:val'), 'single'); left_b.set(qn('w:sz'), '24'); left_b.set(qn('w:color'), border_color)
        borders.append(left_b)
        for side in ['top', 'bottom', 'right']:
            b = OxmlElement(f'w:{side}'); b.set(qn('w:val'), 'none'); borders.append(b)
        tcPr.append(borders)

        # Ghi nội dung đệ quy vào trong khối bọc Callout
        p = cell.paragraphs
